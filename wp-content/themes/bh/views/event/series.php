				...
				foreach ($series as $event) :
					...
					echo '<div class="row event">';
						...
						if ($show_price) :
							...
							echo '<div class="col-xs-4 col-sm-3 event-btn">' . ( ($date >= $today && $ticket_net_link) ? '<a class="btn green-btn event-ticket-net-link small" href="javascript:window.open(\'' . $ticket_net_link . '\',\'Tickets Purchase\',\'width=900,height=550\')">' . __('Purchase', 'BH') . '</a>' : '' ) . '</div>';
						endif;
					echo '</div>';
				endforeach;
