<?php
	...
	$post = new PigArray($_SESSION['microfilm-post'], '');
	    ...
	    '<table>'.
	        '<tbody>'.
	            ...
	            '<tr>'.
	                ...
	                '<td>' . $_SESSION['payment_data']['transactionID'] . '</td>'.
	            '</tr>'.
	            '<tr>'.
	                '<td>Credit Company</td>'.
	                '<td>' . $_SESSION['payment_data']['cc_name'] . '</td>'.
	            '</tr>'.
	            '<tr>'.
	                '<td>Total</td>'.
	                '<td>' . strval( $_SESSION['payment_data']['total']/100 ) . ' NIS </td>'. /* . strtoupper( $pelecardApi->getCurrencyName( $pelecardResponse->getCurrencyCode() ) ) . '</td>'.*/
	            '</tr>'.
	        '</tbody>'.
	    '</table>';
	...
	if ($sentmail) {
	                ...
	                '<div style="background-color:#eeeeee; color: #666666; padding: 5px; text-align:center; font-size:14px;">'.
	                    '<div style="padding: 0 0 5px; display:inline-block; text-align:left;">'.
	                        '<div style="background-color:#ffffff; margin: 10px auto 15px auto; padding: 5px;">'.
	                            ...
	                            'Dear Visitor,<br />'.
	                            '<br />'.
	                            'Thank you for your order no. ' . $_SESSION['payment_data']['transactionID'] . ' of records from the LDS microfilms collection at Beit Hatfutsot.<br />'.
	                            ...
	                            'שלום רב,<br />'.
	                            '<br />'.
	                            'תודה ששלחתם הזמנה מס\' ' . $_SESSION['payment_data']['transactionID'] . ' של צילומי תעודות מאוסף המיקרופילמים  LDS.<br />'.
	    ...
	    $response = array(
	        'error' => array(
	            'code'      => 2,
	            'message'   => 'We had a technical problem. Please try again later.'
	        )
	    );
	}
