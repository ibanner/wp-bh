<div id="microfilm-form" ng-app="microfilmApp" ng-controller="mfController as ctrl" ng-init="getMicrofilmData()">
    ...
    <form name="form" class="bh-form" rc-submit="submitForm()" novalidate>
        ...
        <div class="instructions top-border">
            <div class="inline bullet">3</div>
            <div class="inline"><?php _e('Payment method', 'BH') ?></div>
        </div>
        <table id="payment">
            <tr>
                <td>
                    <label for="cardnumber">*<?php _e('Credit Card Number', 'BH') ?></label>
                    <input name="cardno" id="cardno" ng-model="senderDetails.cardno" required />
                    <div ng-class="{'validation-icon-invalid': rc.form.needsAttention(form.cardno), 'validation-icon-valid': !rc.form.needsAttention(form.cardno) && rc.form.attempted}"></div>
                    <div class="error" ng-show="rc.form.needsAttention(form.cardno)"><?php _e('Required', 'BH') ?></div>
                    <div class="error" ng-show="error.code == 1"><?php _e('There was a problem with the credit card information. Pease make sure credit card info is correct.', 'BH') ?></div>
                </td>

                <td>
                    <label for="cvv">
                        *<?php _e('CVV (3 digit code on back of card)', 'BH') ?><img class="qstn" show-on-hover="explain-cvv" alt="explain cvv" src="<?php echo get_bloginfo('template_directory') . '/images/general/qstn.svg'; ?>" />
                    </label>
                    <input name="cvv" id="cvv" ng-model="senderDetails.cardcvv" required />
                    <div ng-class="{'validation-icon-invalid': rc.form.needsAttention(form.cvv), 'validation-icon-valid': !rc.form.needsAttention(form.cvv) && rc.form.attempted}"></div>
                    <div class="error" ng-show="rc.form.needsAttention(form.cvv)"><?php _e('Required', 'BH') ?></div>
                </td>
                <td>
                    <img class="explain-cvv" src="<?php echo get_bloginfo('template_directory') . '/images/general/cvv.png'; ?>">
                </td>
            </tr>
            <tr>
                <?php if (ICL_LANGUAGE_CODE == 'he') : ?> 
				<td>
                    <label for="cardid">*<?php _e('Credit Card Holder ID Number', 'BH') ?></label>
                    <input name="cardid" id="cardid" ng-model="senderDetails.cardid" required />
                    <div ng-class="{'validation-icon-invalid': rc.form.needsAttention(form.cardid), 'validation-icon-valid': !rc.form.needsAttention(form.cardid) && rc.form.attempted}"></div>
                    <div class="error" ng-show="rc.form.needsAttention(form.cardid)"><?php _e('Required', 'BH') ?></div>
                </td>
                <?php endif; ?>
				<td>
                    <div class="carddate-wrapper">
                        <label for="cardmonth">*<?php _e('Expiration Month', 'BH') ?></label>
                        <select name="cardmonth" id="cardmonth" ng-model="senderDetails.cardmonth" ng-options="month for month in ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']" required></select>
                        <div ng-class="{'validation-icon-invalid': rc.form.needsAttention(form.cardmonth), 'validation-icon-valid': !rc.form.needsAttention(form.cardmonth) && rc.form.attempted}"></div>
                        <div class="error" ng-show="rc.form.needsAttention(form.cardmonth)"><?php _e('Required', 'BH') ?></div>
                    </div>
                    <div class="carddate-wrapper">
                        <label for="cardyear">*<?php _e('Expiration Year', 'BH') ?></label>
                        <select name="cardyear" id="cardyear" ng-model="senderDetails.cardyear" ng-options="year for year in years" required></select>
                        <div ng-class="{'validation-icon-invalid': rc.form.needsAttention(form.cardyear), 'validation-icon-valid': !rc.form.needsAttention(form.cardyear) && rc.form.attempted}"></div>
                        <div class="error" ng-show="rc.form.needsAttention(form.cardyear)"><?php _e('Required', 'BH') ?></div>
                    </div>
                </td>
            </tr>
        </table>
