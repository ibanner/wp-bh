 ...
 * @return		N/A
 */
function awpf_init_categories( &$categories ) {
 ...
 * @param		$taxonomy_term_id	(int) term ID
 * @param		&$min_price			(int) minimum filter price
 * @param		&$max_price			(int) maximum filter price
 * @param		&$min_handle_price	(int) minimum filter handle price
 * @param		&$max_handle_price	(int) maximum filter handle price
 * @param		&$taxonomies		(array) holds array of arrays
 * @param		&$products			(array) holds array of arrays
 * @return		N/A
 */
function awpf_init_products_filter_values( $taxonomy, $taxonomy_term_id, &$min_price, &$max_price, &$min_handle_price, &$max_handle_price, &$taxonomies, &$products ) {
	...
	if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post();
		...
		awpf_update_filter_by_product( $post, $min_price, $max_price, $min_handle_price, $max_handle_price, $taxonomies, $categories, $products );
...
/**
 * awpf_update_filter_by_product
 *
 * Update $products and $taxonomies by a single product meta data
 *
 * @since		1.0
 ...
 * @param		&$max_handle_price	(int) maximum filter handle price
 * @param		&$taxonomies		(array) holds array of arrays
 * @param		&$categories		(array) holds array of arrays
 * @param		&$products			(array) holds array of arrays
 * @return		N/A
 */
function awpf_update_filter_by_product( $post, &$min_price, &$max_price, &$min_handle_price, &$max_handle_price, &$taxonomies, &$categories, &$products ) {
