<?php
/**
 * Plugin Name: oik Weight/Country Shipping Express
 * Plugin URI: http://www.oik-plugins.com/oik-plugins/oik-weightcountry-shipping
 * Description: WooCommerce extension for Weight/Country express shipping
 * Version: 1.0.8
 * Author: bobbingwide (Duplicated by Nir Goldberg)
...
add_action( 'plugins_loaded', 'init_oik_shipping_express', 0 );
 ...
 * 
 */
function init_oik_shipping_express() {
   ...
   *  
   */
	class OIK_Shipping_Express extends WC_Shipping_Method {
  
    /**
     * Constructor for OIK_Shipping_Express class
     *
     * Sets the ID to 'awd_shipping_express'
     * 
     */
		function __construct() {
			$this->id           = 'awd_shipping_express'; // Retain the original code rather than use 'oik_shipping';
			$this->method_title = __( 'Weight/Country Express', 'oik-weightcountry-shipping' );
			...
			add_action( 'woocommerce_update_options_shipping_awd_shipping_express', array( &$this, 'process_admin_options' ) );
      ...
      $woocommerce = function_exists('WC') ? WC() : $GLOBALS['woocommerce'];
			...
			$this->form_fields = array(
				...
				'title'      => array(
					...
					'default'     => __( 'Express Shipping', 'oik-weightcountry-shipping' ),
				),
				'tax_status' => array(
 ...
 */  
function add_oik_shipping_express( $methods ) {
	$methods[] = 'OIK_Shipping_Express';
	return $methods;
}

add_filter( 'woocommerce_shipping_methods', 'add_oik_shipping_express' );
